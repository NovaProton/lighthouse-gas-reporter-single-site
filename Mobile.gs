function runLighthouseAuditMobile() {
  ...
  const lighthouseApiUrl = `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${encodeURIComponent(targetUrl)}&category=performance&category=accessibility&category=seo&category=best-practices&strategy=mobile&key=${API_KEY}`;
  ...
  const mobileSheet = SpreadsheetApp.openById(sheetId).getSheetByName(sheetName);
  if (!mobileSheet) {
    throw new Error(`Sheet named ${sheetName} not found in spreadsheet ID ${sheetId}`);
  }

  mobileSheet.appendRow([
    metrics.Date,
    metrics.Time,
